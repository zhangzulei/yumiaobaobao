...
public interface Serviceindex_discover {
    ...
    List<index_discover> AllLocation(Double Latitude, Double Longitude,int pagenum,int pagesize);

    index_discover oneTemplate( String templateID);

    List<index_discover> fuzzyQuery( String content);
    ...
    List<index_discover> selectAll(int pagenum,int pagesize);

    List<index_discover> selectByUseridLast(Integer userid);
    index_discover selectCollection(Integer Noteid);

    List<index_discover> recommendNotes( Integer talkingid);
