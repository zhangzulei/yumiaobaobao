...
//            UserInfo userInfo = new UserInfo();
    ...
    @ApiOperation(value = "评论列表", notes = "评论列表 message 0=>失败  1=>成功")
    public String commentList(){
        List<nice_comment> commentLists = favoService.selectCommentList();
        if(commentLists==null||commentLists.size()<=0){
            return "{\"Message\": \"0\"}";
        }
        return "{\"Message\": \"1\",\"Data\":"+ JSON.toJSONString(commentLists)+"}";
    }
